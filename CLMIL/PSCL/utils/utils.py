...
def collate_MIL_select(batch):
	img = torch.cat([item[0] for item in batch], dim = 0)
	coords = np.vstack([item[2] for item in batch])
	label = torch.LongTensor([item[1] for item in batch])
	bag_name = np.vstack([item[3] for item in batch])

	return [img, label, coords, bag_name]
...
def collate_features_with_path(batch):
	img = torch.cat([item[0] for item in batch], dim = 0)
	coords = np.vstack([item[1] for item in batch])
	paths = np.array([item[2] for item in batch])
	paths = np.hstack(paths)
	return [img, coords, paths]
...
def get_split_loader_select(split_dataset, training=False, testing=False, weighted=False):
	"""
		return either the validation loader or training loader
	"""
	kwargs = {'num_workers': 4} if device.type == "cuda" else {}
	if not testing:
		if training:
			if weighted:
				...
				loader = DataLoader(split_dataset, batch_size=1, sampler=WeightedRandomSampler(weights, len(weights)),
									collate_fn=collate_MIL_select, **kwargs)
			else:
		...
		ids = np.random.choice(np.arange(len(split_dataset), int(len(split_dataset) * 0.1)), replace=False)
		loader = DataLoader(split_dataset, batch_size=1, sampler=SubsetSequentialSampler(ids), collate_fn=collate_MIL_select(),
							**kwargs)
