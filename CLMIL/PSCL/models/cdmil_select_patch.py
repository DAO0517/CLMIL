...
class MILNet(nn.Module):
    ...
    def forward(self, x, label, coords, bag_path):  #, image_paths, label
        ...
        if self.num_classes > 2:
            if cf[cf_index[0]] > cf[cf_index[1]] and cf_index[0]==similarity_index[0].cuda():
                ...
                for i in range(self.num_classes):  # num_tiles1的数量多于num_tiles2
                    if i == cf_index[0]:  # 预测概率最高的一类
                        coord_selects = coords[m_indices[0: num_tiles1, i]]
                        for coord_select in coord_selects:
                            coord = coord_select.detach().cpu().numpy()
                            image_path = os.path.join(bag_path, str(coord[0]) + '_' + str(coord[1]) + '.png')
                            if os.path.exists(image_path):
                                data = {'case_id': int(self.case_id), 'image_path': image_path, 'label': i}
                                self.case_id += 1
                                self.combined_data = self.combined_data.append(data, ignore_index=True)
            else:
                ...
                # g_feats = torch.cat(g_feats, dim=0)[None,]
                coord_selects = coords[m_indices[0: num_tiles2, label]]
                for coord_select in coord_selects:
                    coord = coord_select[0].detach().cpu().numpy()
                    image_path = os.path.join(bag_path, str(coord[0]) + '_' + str(coord[1]) + '.png')
                    if os.path.exists(image_path):
                        data = {'case_id': int(self.case_id), 'image_path': image_path, 'label': int(label)}
                        self.case_id += 1
                        self.combined_data = self.combined_data.append(data, ignore_index=True)
        else:
            if cf[0] > cf[1] and cf_index[0]==similarity_index[0].cuda():
                ...
                g_feats = torch.cat(g_feats, dim=0)[None,]
                coord_selects = coords[m_indices[0: num_tiles1, 0]]
                for coord_select in coord_selects:
                    coord = coord_select[0].detach().cpu().numpy()
                    image_path = os.path.join(bag_path, str(coord[0]) + '_' + str(coord[1]) + '.png')
                    data = {'case_id': self.case_id, 'image_path': image_path, 'label': 0}
                    self.case_id += 1
                    self.combined_data = self.combined_data.append(data, ignore_index=True)
                ...
                for coord_select in coord_selects:
                    coord = coord_select[0].detach().cpu().numpy()
                    image_path = os.path.join(bag_path, str(coord[0]) + '_' + str(coord[1]) + '.png')
                    data = {'case_id': self.case_id, 'image_path': image_path, 'label': 1}
                    self.case_id += 1
                    self.combined_data = self.combined_data.append(data, ignore_index=True)
            else:
                g_feats = []
